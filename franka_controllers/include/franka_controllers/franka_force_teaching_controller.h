...
#include <franka_controllers/desired_mass_paramConfig.h>
...
class FrankaForceTeachingController : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  // Dynamic reconfigure
  std::unique_ptr<dynamic_reconfigure::Server<franka_controllers::desired_mass_paramConfig>>
      dynamic_server_desired_mass_param_;
  ros::NodeHandle dynamic_reconfigure_desired_mass_param_node_;
  void desiredMassParamCallback(franka_controllers::desired_mass_paramConfig& config,
                                uint32_t level);
};
